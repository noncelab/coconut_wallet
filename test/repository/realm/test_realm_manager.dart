...
class TestRealmManager implements RealmManager {
  ...
  // path 없이 Configuration.inMemory()를 쓰면 모든 인스턴스가 이름 없는 같은 in-memory DB를 공유한다.
  // 이 공유 범위는 Dart isolate가 아니라 "실행 중인 프로세스 전체"라서, flutter test가
  // 여러 테스트 파일을 각자 isolate로 나눠 동시에 돌려도 서로 격리되지 않는다.
  // 그 결과 한 파일의 tearDown(realm.close())이 아직 같은 DB를 쓰고 있는 다른 파일의 테스트를 깨뜨린다
  // (RealmException: invalidated or deleted object).
  ...
  void reset() {
    realm.write(() {
      ...
      realm.deleteAll<RealmHotWalletMetadata>();
