...
void main() {
  ...
  group('updateScriptStatusList 테스트', () {
    test('새로운 스크립트 상태 추가 테스트', () async {
      ...
      final beforeStatuses =
          realmManager.realm.query<RealmScriptStatus>(r'walletId == $0', [testWalletItem.id]).toList();

      expect(beforeStatuses.length, 0);

      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, newScriptStatuses);
      ...
      final firstStatus = savedStatuses.firstWhere(
        (status) => status.scriptPubKey == newScriptStatuses[0].scriptPubKey,
      );
      expect(firstStatus.status, newScriptStatuses[0].status);

      final secondStatus = savedStatuses.firstWhere(
        (status) => status.scriptPubKey == newScriptStatuses[1].scriptPubKey,
      );
    ...
    test('기존 스크립트 상태 업데이트 테스트', () async {
      ...
      final updatedScriptStatuses = [ScriptStatusMock.createMockScriptStatus(testWalletItem, 0, status: "newStatus")];

      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, updatedScriptStatuses);
    ...
    test('업데이트 필요 없는 경우 테스트', () async {
      ...
      final sameScriptStatuses = [mockScriptStatus];

      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, sameScriptStatuses);
    ...
    test('지갑이 삭제된 경우 관련 스크립트 상태 삭제 테스트', () async {
      ...
      final scriptStatuses = [ScriptStatusMock.createMockScriptStatus(testWalletItem, 0, status: "newStatus")];

      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, scriptStatuses);

      // Then
      expect(result.isSuccess, true);

      final deletedStatuses =
          realmManager.realm.query<RealmScriptStatus>(r'walletId == $0', [testWalletItem.id]).toList();
    ...
    test('추가와 업데이트가 혼합된 경우 테스트', () async {
      ...
      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, mixedScriptStatuses);
    ...
    test('빈 스크립트 상태 목록 처리 테스트', () async {
      ...
      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, emptyScriptStatuses);
    ...
    test('동일한 scriptPubKey에 대한 status 변경 감지 테스트', () async {
      ...
      final updatedMockStatus = ScriptStatusMock.createMockScriptStatus(testWalletItem, 0, status: "changedStatus");

      // When
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, [updatedMockStatus]);
    ...
    test('여러 스크립트에 대한 부분적 업데이트 테스트', () async {
      ...
      final existingMock1 = ScriptStatusMock.createMockScriptStatus(testWalletItem, 0, status: "status1");
      final existingMock2 = ScriptStatusMock.createMockScriptStatus(testWalletItem, 1, status: "status2");
      final existingMock3 = ScriptStatusMock.createMockScriptStatus(testWalletItem, 2, status: "status3");
      ...
      final partialUpdateStatuses = [
        ...
        ScriptStatusMock.createMockScriptStatus(testWalletItem, 1, status: "updatedStatus2"), // 변경된 상태
        existingMock3, // 동일한 상태
      ];

      // When: 스크립트 상태 업데이트 실행
      final result = await subscriptionRepository.updateScriptStatusList(testWalletItem.id, partialUpdateStatuses);
  ...
  group('deleteScriptStatusIfWalletDeleted 테스트', () {
    ...
    test('여러 지갑 중 특정 지갑만 삭제된 경우 테스트', () async {
      ...
      // Then
      final firstWalletStatuses =
          realmManager.realm.query<RealmScriptStatus>(r'walletId == $0', [testWalletItem.id]).toList();
      expect(firstWalletStatuses.length, 0);

      final secondWalletStatuses =
          realmManager.realm.query<RealmScriptStatus>(r'walletId == $0', [secondWalletId]).toList();
