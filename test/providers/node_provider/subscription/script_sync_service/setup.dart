...
class _ScriptSyncTestSetup {
  static void setupNetworkAndSharedPrefs(_ScriptSyncTestData testData) {
    ...
    when(
      testData.sharedPrefsRepository.setInt(SharedPrefKeys.kNextIdField, testData.walletA.id + 1),
    ).thenAnswer((_) async => true);
    when(
      testData.sharedPrefsRepository.setInt(SharedPrefKeys.kNextIdField, testData.walletB.id + 1),
    ).thenAnswer((_) async => true);
  }
  ...
  static Future<void> setupLocalDatabase(_ScriptSyncTestData testData) async {
    ...
    await ScriptSyncServiceMock.addressRepository.ensureAddressesInit(walletItemBase: testData.walletA);
    await ScriptSyncServiceMock.addressRepository.ensureAddressesInit(walletItemBase: testData.walletB);
  }
  ...
  static Future<void> setupRbfCpfpInitialEnvironment(_ScriptSyncTestData testData) async {
    ...
    ScriptSyncServiceMock.utxoRepository.addAllUtxos(testData.walletA.id, [
      UtxoState(
        ...
        derivationPath: '${testData.walletA.walletBase.derivationPath}/0/${_TestConstants.addressIndex}',
  ...
  static Future<void> setupCpfpEnvironment(_ScriptSyncTestData testData) async {
    ...
    final cpfpTxHistoryRes = GetTxHistoryRes(height: 0, txHash: testData.cpfpTx!.transactionHash);
