...
2. 모니터링 중인 주소 확인 가능
### Fixed
1. coconut_lib 1.1.1 예외 반환 방식이 달라짐. 이에 맞게 트랜잭션 생성 시 예외 처리 로직 변경
2. 합치기 조건 확인 코드 잘못된 주석 처리 제거
...
## mainnet 0.12.3, regtest 3.12.3
• 수량 입력 시 소수점 구분자로 마침표(.) 외에 쉼표(,)도 사용할 수 있도록 개선했습니다.
• 합치기(Merge) 기능에서 수수료율이 0 설정 안되도록 개선
(regtest 버전 업데이트 실수 3.11.2 -> 3.12.3)

## mainnet 0.12.2, regtest 3.11.2
### Fixed
* 가짜 잔액 1000BTC이상 설정 안됨
* 임시 저장 트랜잭션 전송 완료 후 홈화면이 아니라 검은 화면으로 이동 후 앱 사용 불가
* 수수료율 입력값과 단위가 겹쳐 보임
* 홈 화면 - 최근 거래 내역 - 같은 금액이 나가고 들어와서 변동 금액이 0인 경우 '변동 없음'으로 안내
* iOS 토스트 스타일 수정
...
* BIP-177 도입
### Enhanced
* 거래 정보 확인, 브로드캐스팅 화면 재구성
### Fixed
* RBF 시 임시저장 트랜잭션 관련 오류 메시지 출력되며 브로드캐스팅 실패 버그 수정

## mainnet 0.8.0, regtest 3.7.0
### Added
* 서명 전 보내기 화면에서 임시저장 / 서명 후 전송하기 화면에서 임시 저장 가능
* 보내기 화면에서 UTXO 선택 모드 전환 UI/UX 개선 (지갑 이름 터치 or 더보기 버튼의 스위치 / 마지막 선택한 모드로 저장됨)
* VPN 연결 감지 시 홈화면에서 안내
### Fixed
* 홈 위젯 버그 수정
ㄴ 사용자가 기간 설정 시 설정한 기간이 화면에 다르게 표시됨
ㄴ 사용자가 기간 설정한 당시 EndDate와 같은 날짜이지만 그 뒤 시간에 발생한 TX가 집계에 누락
ㄴ 24시간 내 거래 내역 : 1시간 아내 거래건이 항상 '방금 전'으로 표기됨
* UtxoListScreen pending UTXO 잠금/잠금 해제 가능하던 버그 수정 / 선택 모드에서 pending UTXO 클릭 시 토스트 안내
* 지갑 추가 - 직접 입력
1) Descriptor QR scan으로 추가 안되던 버그 수정 
2) ExtendedPublicKey QR Scan 또는 붙여넣기 시 mfp 입력 바텀싯 보이는 과정 추가
