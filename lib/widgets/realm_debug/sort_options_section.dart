...
class SortOptionsSection extends StatelessWidget {
  ...
  const SortOptionsSection({super.key, required this.queryController});
  ...
  Widget _buildSortToggle(BuildContext context, RealmDebugViewModel viewModel) {
    return Row(
      children: [
        const Text('정렬 옵션', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(width: 8),
        Switch(value: viewModel.enableSort, onChanged: (value) => viewModel.toggleSortEnabled(value)),
      ],
    );
  }
            ...
            children: [
              ...
              Expanded(
                child: DropdownButton<String>(
                  value: viewModel.selectedSortField.isNotEmpty ? viewModel.selectedSortField : null,
                  hint: const Text('필드 선택'),
                  isExpanded: true,
                  items:
                      viewModel.getSortableFields().map((field) {
          ...
          Row(
            children: [
              ...
              ToggleButtons(
                ...
                children: const [
                  Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('오름차순')),
                  Padding(padding: EdgeInsets.symmetric(horizontal: 16), child: Text('내림차순')),
                ],
              ),
            ],
          ),
