...
import 'package:coconut_wallet/widgets/features/settings/realm_debug/sort_options_section.dart';
import 'package:coconut_wallet/widgets/features/settings/realm_debug/statistics_section.dart';
...
class QueryInputSection extends StatelessWidget {
          ...
          child:
              viewModel.isLoading
                  ? Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        const Spacer(),
                        Text('로딩 중...', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                      ],
                    ),
                  )
                  : DropdownButton<String>(
                    ...
                    isExpanded: true,
                    items:
                        viewModel.availableTables.map((table) {
  ...
  Widget _buildQueryExamples(BuildContext context, RealmDebugViewModel viewModel) {
    return Wrap(
      spacing: 8,
      children:
          viewModel.getQueryExamples().map((example) {
            return ActionChip(
              label: Text(
