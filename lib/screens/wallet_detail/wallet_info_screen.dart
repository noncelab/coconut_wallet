...
class _WalletInfoScreenState extends State<WalletInfoScreen> {
    ...
    if (context.mounted) {
      widget.entryPoint == kEntryPointWalletHome
          ? Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(builder: (BuildContext context) => const AppGuard(child: WalletHomeScreen())),
            (route) => false,
          )
  ...
  void _showExtendedBottomSheet(String extendedPublicKey) {
    CommonBottomSheets.showBottomSheet_90(
      context: context,
      child: AppGuard(child: QrcodeBottomSheet(qrData: extendedPublicKey, title: t.extended_public_key)),
    );
  }
}
