...
class WalletHomeScreen extends StatefulWidget {
  ...
  static void openAddWalletIfActive() =>
      _currentState?._showAddWalletMenu(_AddWalletMenuMode.walletType);
...
class _WalletHomeScreenState extends State<WalletHomeScreen>
  ...
  double _walletFilterDragDistance = 0;
    ...
    bool? result = await showDialog<bool>(
      ...
      builder:
          (context) => CoconutPopup(
            ...
            title: t.long_pressed_menu.hide_home_widget_title(
              widgetName: _getWidgetName(type),
            ),
    ...
    >(
      ...
      >(
        ...
        builder: (context, data, child) {
          final viewModel = Provider.of<WalletHomeViewModel>(
            context,
            listen: false,
          );
          ...
          final hasEnabledHomeFeature = homeFeatures.any(
                ...
                child: Listener(
                  ...
                  onPointerMove: (event) {
                    if (_lastPointerDownPosition != null &&
                        !_didDragSincePointerDown) {
                      final delta =
                          (event.position - _lastPointerDownPosition!).distance;
                      if (delta > 5.0) {
                  ...
                  onPointerUp: (_) {
                    if (!_didDragSincePointerDown &&
                        _viewModel.isEditWidgetMode) {
      ...
      if (insertedIndexes.isNotEmpty) {
        if (_previousWalletList.isEmpty && _isFirstLoad) {
          ...
          for (var i = 0; i < insertedIndexes.length; i++) {
            _walletListKey.currentState?.insertItem(
              insertedIndexes[i],
              ...
              child: Column(
                ...
                children: [
                  ...
                  Shimmer.fromColors(
                    ...
                    highlightColor:
                        context.coconutColors.surfaceSkeletonHighlight,
                    child: Container(
                        ...
                        builder: (context, data, child) {
                          ...
                          final balance =
                              _viewModel.fakeBalanceTotalAmount != null
                                  ...
                                  : Map.fromEntries(
                                        _viewModel.walletBalanceMap.entries
                                            .where(
                                              (entry) =>
                    ...
                    return Row(
                      ...
                      children: [
                                    ...
                                    : FittedBox(
                                      ...
                                      >(
                                        ...
                                        builder: (context, excludedIds, child) {
                                          ...
                                          final prevValue = filteredBalanceMap
                                              .values
                                              .map((e) => e.previous)
                                              .fold(
                                                0,
                        ...
                        ShrinkAnimationButton(
                          ...
                          pressedColor:
                              context.coconutColors.homeSurfaceCardPressed,
                          onPressed: () {
  ...
  void _onTapReceive(List<int> walletOrder) {
    ...
    Navigator.of(
      context,
    ).pushNamed("/receive-address", arguments: {"id": targetId});
  }

  Future<void> _onTapSend(
    List<int> walletOrder, {
    String? bitcoinUri,
    bool shouldBypassSyncCheck = false,
  }) async {
    final firstWallet = _viewModel.walletItemList.firstOrNull;
    if (firstWallet == null) {
  ...
  Widget _buildWalletOverview(
    ...
    final walletFilterOrder =
        preferenceProvider.walletFilterOrder
            .where(preferenceProvider.isWalletFilterVisible)
            .toList();
          ...
          children: [
            Row(
              children: [
                for (
                  var index = 0;
                  index < walletFilterOrder.length;
                  index++
                ) ...[
                  _buildWalletFilterChip(
                    walletFilterOrder[index],
                    _getWalletFilterLabel(walletFilterOrder[index]),
                  ),
                  if (index < walletFilterOrder.length - 1)
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  _buildWalletOverviewActions(hasWallets: wallets.isNotEmpty),
                ],
              ),
            ),
          ],
  ...
  Widget _buildWalletFilterChip(WalletFilter filter, String label) {
    final isSelected = _walletFilter == filter;
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () => _selectWalletFilter(filter),
  ...
  Widget _buildSwipeableWalletList(
    ...
    final visibleWallets = switch (_walletFilter) {
      WalletFilter.all => wallets,
      WalletFilter.watchOnly =>
        wallets.where((wallet) => !wallet.hasLocalKey).toList(),
      WalletFilter.hot =>
        wallets.where((wallet) => wallet.hasLocalKey).toList(),
    };
    final visibleWalletIds = visibleWallets.map((wallet) => wallet.id).toSet();
    final lastVisibleWalletId = visibleWallets.lastOrNull?.id;
    final walletList = Column(
      children: [
        ...wallets.map((wallet) {
          final isVisible = visibleWalletIds.contains(wallet.id);
          return AnimatedSize(
            ...
            child: AnimatedSwitcher(
              ...
              child:
                  isVisible
                      ? KeyedSubtree(
                        key: ValueKey('visible-${wallet.id}'),
                        child: _buildWalletItem(
                          wallet,
                          kAlwaysCompleteAnimation,
                          walletBalanceMap[wallet.id] ??
                              AnimatedBalanceData(0, 0),
                          getFakeBalance(wallet.id),
                          wallet.id == lastVisibleWalletId,
                          context.coconutColors.homeSurfaceCard,
                        ),
                      )
                      : SizedBox.shrink(key: ValueKey('hidden-${wallet.id}')),
            ),
          );
        }),
      ],
    ...
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onHorizontalDragStart: (_) => _walletFilterDragDistance = 0,
      onHorizontalDragUpdate: (details) {
        _walletFilterDragDistance += details.delta.dx;
      },
      onHorizontalDragEnd: _handleWalletFilterDragEnd,
      child: AnimatedSize(
        duration: const Duration(milliseconds: 220),
        curve: Curves.easeOutCubic,
        alignment: Alignment.topCenter,
        child: AnimatedSwitcher(
          ...
          child:
              visibleWallets.isEmpty
                  ? KeyedSubtree(
                    key: const ValueKey('empty-wallet-list'),
                    child: _buildEmptyWalletOverview(),
                  )
                  : KeyedSubtree(
                    key: const ValueKey('non-empty-wallet-list'),
                    child: Padding(
  ...
  void _handleWalletFilterDragEnd(DragEndDetails details) {
    const distanceThreshold = 48.0;
    const velocityThreshold = 350.0;
    final velocity = details.primaryVelocity ?? 0;

    if (_walletFilterDragDistance <= -distanceThreshold ||
        velocity <= -velocityThreshold) {
      _moveWalletFilter(1);
    } else if (_walletFilterDragDistance >= distanceThreshold ||
        velocity >= velocityThreshold) {
      _moveWalletFilter(-1);
    }
    _walletFilterDragDistance = 0;
  }

  void _moveWalletFilter(int offset) {
    final preferenceProvider = context.read<PreferenceProvider>();
    final filterOrder =
        preferenceProvider.walletFilterOrder
            .where(preferenceProvider.isWalletFilterVisible)
            .toList();
    final currentIndex = filterOrder.indexOf(_walletFilter);
    final nextIndex = (currentIndex + offset).clamp(0, filterOrder.length - 1);
    _selectWalletFilter(filterOrder[nextIndex]);
  }

  void _selectWalletFilter(WalletFilter filter) {
    if (_walletFilter == filter) return;
    setState(() => _walletFilter = filter);
  }

  Widget _buildEmptyWalletOverview() {
    final addWalletIconSrc = switch (_walletFilter) {
    ...
    final addWalletDescription = switch (_walletFilter) {
      WalletFilter.all => t.wallet_home_screen.add_wallet_type_all,
      WalletFilter.watchOnly => t.wallet_home_screen.add_wallet_type_watch_only,
      WalletFilter.hot => t.wallet_home_screen.add_wallet_type_hot,
    };
  ...
  Widget _buildWalletOverviewActions({required bool hasWallets}) {
    return SizedBox(
      height: 57,
      child: Row(
        children: [
          Expanded(
            child: ShrinkAnimationButton(
              ...
              child: Center(
                child: Text(
                  t.wallet_home_screen.add_wallet,
                  style: CoconutTypography.body2_14_Bold.setColor(
                    context.coconutColors.primaryText,
                  ),
                ),
  ...
  Widget _getWalletRowItem(
    ...
    return Selector2<
      PreferenceProvider,
      WalletHomeViewModel,
      Tuple2<BitcoinUnit, bool>
    >(
      selector:
          (_, preferenceProvider, viewModel) =>
              Tuple2(preferenceProvider.currentUnit, viewModel.isBalanceHidden),
      builder: (context, data, child) {
  ...
  List<LongPressedMenuItem> _buildWalletMenuItems(WalletItemBase walletItem) {
    ...
    return [
      ...
      LongPressedMenuItem(
        ...
        iconPath: 'assets/svg/star-outlined.svg',
    ...
    showDialog(
      ...
      builder:
          (dialogContext) => CoconutPopup(
            ...
            onTapRight:
                () =>
                    _authenticateBeforeWalletDeletion(dialogContext, walletId),
          ),
    );
  }

  Future<void> _authenticateBeforeWalletDeletion(
  ...
  Widget _buildFeatureSectionIfEnabled(
    HomeFeatureType type,
    Widget Function() builder,
  ) {
    if (_viewModel.isHomeFeatureEnabled(type)) {
      return SliverToBoxAdapter(
        child: Column(children: [builder(), CoconutLayout.spacing_400h]),
      );
    }
          ...
          child: Column(
            children: [
                ...
                child: Center(
                  child: Selector2<
                    ...
                    Tuple3<
                      BitcoinUnit,
                      bool,
                      List<Tuple2<int, TransactionRecord>>
                    >
                  >(
                    selector:
                      ...
                      if (ordered.isEmpty ||
                        ...
                        return Container(
                          ...
                          child: _buildEmptyRecentTransactions(
                            (_viewModel.shouldShowLoadingIndicator &&
                                    _viewModel.walletItemList.isNotEmpty) ||
              ...
              Selector<WalletHomeViewModel, int>(
                selector:
                    (_, viewModel) =>
                        viewModel.orderedRecentTransactions.length,
                      ...
                      child: Row(
                        ...
                        children: List.generate(indicatorCount, (index) {
                          return AnimatedContainer(
                            ...
                            width:
                                _recentTransactionCurrentPage == index ? 12 : 8,
                            height: 8,
                            decoration: BoxDecoration(
  ...
  Widget _buildRecentTransactionCard(
    ...
    Widget buildTxRow(TransactionRecord transaction) {
      ...
      final List<String> transactionTimeStamp = DateTimeUtil.formatTimestamp(
        txDate,
      );
      final String formattedAmount = currentUnit.formatAmountWithSign(
        transaction.amount,
      ...
      final iconColor = switch (status) {
        ...
        TransactionStatus.sending => context.coconutColors.sendingColor,
        TransactionStatus.received ||
        TransactionStatus.receiving => context.coconutColors.receivingColor,
  ...
  Widget _buildEmptyRecentTransactions(bool isSyncing) {
    ...
    } else if (_showEmptyRecentTransactionWidget &&
        _recentTransactionBannerTimer == null) {
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Center(
                          child: Text(
                            stayHumbleStackSats,
                            style: CoconutTypography.body1_16_NumberBold
                                .setColor(context.coconutColors.tertiaryText),
                          ),
                        ),
                ...
                ] else if (_viewModel.recentTransactionAnalysis?.isEmpty ==
                    false) ...[
                  Selector<PreferenceProvider, BitcoinUnit>(
                    selector: (_, viewModel) => viewModel.currentUnit,
                    builder: (context, currentUnit, child) {
          ...
          Text(
            _viewModel.analysisPeriod != 0
                ? t.wallet_home_screen.analysis_period(
                  ...
                  transaction_type:
                      _viewModel.selectedAnalysisTransactionTypeName,
                )
                      ...
                      children: [
                        if (mode == _AddWalletMenuMode.walletType) ...[
                          ...
                          Row(
                            ...
                            _AddWalletMenuMode.watchOnlySource) ...[
                          Row(
  ...
  SliverAppBar _buildAppBar(NetworkStatus networkStatus) {
    ...
    final addWalletIconPath = switch (context
        ...
        child:
            shouldShow
                ? Row(
                  ...
                  children: [
                    SvgPicture.asset(
                      'assets/svg/cloud-disconnected.svg',
                      width: 16,
                    ),
                    CoconutLayout.spacing_100w,
                    SizedBox(
      ...
      child: AnimatedSwitcher(
        ...
        child:
            viewModel.shouldShowLoadingIndicator &&
                    viewModel.walletItemList.isNotEmpty
                ? const Center(
                  child: Padding(
        ...
        builder: (context, isVisible, child) {
          return Visibility(
            ...
            child: CoconutPulldownMenu(
              ...
              shadowColor: context.coconutColors.shadowDefault.withValues(
                alpha: 0.06,
              ),
