...
class LabelExportViewModel extends ChangeNotifier {
  ...
  List<Bip329Record> _getRecordsForWallet(int walletId) {
    ...
    return _converter.generateRecordsForWallet(
  ...
  bool hasExportableLabelsForWallet(int walletId) {
    return _getRecordsForWallet(walletId).isNotEmpty;
  }
  ...
  Future<XFile?> exportLabelsForWallet(int walletId) async {
    final records = _getRecordsForWallet(walletId);
    if (records.isEmpty) {
      return null;
    }
  ...
  Future<LabelExportResult> exportLabelsForWallets(List<int> walletIds) async {
    final List<Bip329Record> allRecords = [];

    for (final walletId in walletIds) {
      allRecords.addAll(_getRecordsForWallet(walletId));
    }
    ...
    final jsonLString = _converter.encodeToJsonL(allRecords);
    final xFile = await _fileService.saveLabelFile(jsonLString);
