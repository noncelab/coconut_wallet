...
description: Check whether pinned SSL certificate fingerprints for default Electrum servers in lib/enums/electrum_enums.dart have changed, and update them if necessary. Run before app deployment if not already executed today.
---
...
3. For each such server, run a command like the following to fetch the current certificate's SHA-256 fingerprint:
   ```bash
